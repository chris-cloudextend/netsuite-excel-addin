...
const FUNCTIONS_VERSION = '3.0.5.98';  // Version marker for debugging
...
// ============================================================================
// TYPEBALANCE - Get balance for all accounts of a specific Account Type
// Similar to BALANCE, but filters by type (e.g., "OthAsset", "Expense") not number
// BS types: cumulative from inception; P&L types: period range
// ============================================================================
/**
 ...
 * @param {any} accountType NetSuite account type (e.g., "OthAsset", "Expense")
 * @param {any} [fromPeriod] Start period (required for P&L, ignored for BS)
 * @param {any} toPeriod End period (required)
 * @param {any} [subsidiary] Subsidiary name or ID (optional)
 * @param {any} [department] Department name or ID (optional)
 * @param {any} [location] Location name or ID (optional)
 * @param {any} [classId] Class name or ID (optional)
 * @param {any} [accountingBook] Accounting Book ID (optional, defaults to Primary Book)
 * @returns {Promise<number>} Total balance for the account type
 */
async function TYPEBALANCE(accountType, fromPeriod, toPeriod, subsidiary, department, location, classId, accountingBook) {
    try {
        console.log(`📊 TYPEBALANCE called with: accountType=${accountType}, fromPeriod=${fromPeriod}, toPeriod=${toPeriod}`);
        ...
        const validTypes = [
            ...
            // P&L
            'Income', 'OthIncome', 'COGS', 'Cost of Goods Sold', 'Expense', 'OthExpense'
        ];
        ...
        const matchedType = validTypes.find(t => t.toLowerCase() === accountTypeStr.toLowerCase());
        if (!matchedType) {
            console.error(`❌ TYPEBALANCE: Invalid account type "${accountTypeStr}". Valid types: ${validTypes.join(', ')}`);
            return 0;
        }
        
        const normalizedType = matchedType;
        
        // ================================================================
        // DETERMINE IF BS OR P&L
        // ================================================================
        const plTypes = ['Income', 'OthIncome', 'COGS', 'Cost of Goods Sold', 'Expense', 'OthExpense'];
        const isBS = !plTypes.includes(normalizedType);
        console.log(`   Account type "${normalizedType}" is BS: ${isBS}`);
        
        // ================================================================
        // CONVERT PERIODS
        // ================================================================
        // toPeriod is required
        if (toPeriod === undefined || toPeriod === null || toPeriod === '') {
        ...
        const convertedToPeriod = convertToMonthYear(toPeriod, false);
        if (!convertedToPeriod) {
            console.error('❌ TYPEBALANCE: Could not parse toPeriod:', toPeriod);
            return 0;
        }
        
        let convertedFromPeriod = '';
        if (isBS) {
            // For BS, ignore fromPeriod (cumulative from inception)
            if (fromPeriod) {
                console.log(`   ⚠️ BS account type - ignoring fromPeriod "${fromPeriod}"`);
            }
            convertedFromPeriod = ''; // Empty = cumulative
        } else {
            // For P&L, fromPeriod defaults to toPeriod if not specified
            if (fromPeriod === undefined || fromPeriod === null || fromPeriod === '') {
                convertedFromPeriod = convertedToPeriod;
                console.log(`   P&L: defaulting fromPeriod to toPeriod "${convertedToPeriod}"`);
            } else {
                convertedFromPeriod = convertToMonthYear(fromPeriod, true); // Jan for year-only
                if (!convertedFromPeriod) {
                    console.error('❌ TYPEBALANCE: Could not parse fromPeriod:', fromPeriod);
                    return 0;
                }
            }
        }
        
        console.log(`📊 TYPEBALANCE: ${normalizedType}`);
        console.log(`   Period: ${convertedFromPeriod || '(inception)'} → ${convertedToPeriod}`);
        ...
        // Build cache key
        const cacheKey = `typebalance:${normalizedType}:${convertedFromPeriod}:${convertedToPeriod}:${subsidiaryStr}:${departmentStr}:${locationStr}:${classIdStr}:${accountingBookStr}`;
        
        // Check cache first
        if (cache.balance.has(cacheKey)) {
            cacheStats.hits++;
            console.log(`📥 CACHE HIT [type balance]: ${normalizedType}`);
            return cache.balance.get(cacheKey);
        }
        
        // Check if there's already a request in-flight for this exact key
        if (inFlightRequests.has(cacheKey)) {
            console.log(`⏳ Waiting for in-flight request [type balance]: ${normalizedType}`);
            return await inFlightRequests.get(cacheKey);
        }
        
        cacheStats.misses++;
        console.log(`📥 Fetching type balance for ${normalizedType}...`);
        ...
        const requestPromise = (async () => {
            try {
                ...
                if (!response.ok) {
                    const errorText = await response.text();
                    console.error(`Type Balance API error: ${response.status}`, errorText);
                    if (response.status === 524 || response.status === 522 || response.status === 504) {
                        return 0; // Return 0 for timeouts, let user retry
                    }
                    return 0;
                }
                
                const data = await response.json();
                console.log(`📨 Type Balance API response:`, JSON.stringify(data));
                
                if (data.value === null || data.value === undefined) {
                    console.error(`❌ Type Balance: API returned null/undefined`);
                    return 0;
                }
                
                const value = parseFloat(data.value);
                if (isNaN(value)) {
                    console.error(`❌ Type Balance: Invalid number: ${data.value}`);
                    return 0;
                }
                
                // Cache the result
                cache.balance.set(cacheKey, value);
                console.log(`✅ Type Balance (${normalizedType}): ${value.toLocaleString()}`);
                
                return value;
                
            } catch (error) {
                console.error('Type Balance fetch error:', error);
                return 0;
            } finally {
                inFlightRequests.delete(cacheKey);
            }
